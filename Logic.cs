...
namespace TechniteLogic
{
    public class Logic
    {
        public static class Helper
        {
            ...
            /// <param name="f">Evaluation function. Must return 0/NotAChoice if not applicable, and >1 otherwise. Higher values indicate higher probability</param>
            /// <returns>Chocen relative cell, or Grid.RelativeCell.Invalid if none was found.</returns>
            public static Grid.RelativeCell EvaluateBeggingForEnergyNeighborTechnites(Grid.CellID location, Func<Grid.RelativeCell, Technite, int> f)
            {
            ...
            /// <param name="f">Evaluation function. Must return 0/NotAChoice if not applicable, and >1 otherwise. Higher values indicate higher probability</param>
            /// <returns></returns>
            public static Grid.RelativeCell EvaluateBeggingChoices(Grid.CellID location, Func<Grid.RelativeCell, Grid.CellID, int> f)
            {
                options.Clear();
                int total = 0;
                foreach (var n in location.GetRelativeNeighbors())
                {
                    ...
                    if (Grid.World.GetCell(cellLocation).content == Grid.Content.Technite)
                    {
                        int q = f(n, cellLocation);  // q = is Technite n begging for Energy
                        if (q > 0)
                        {
                            total += q;
                            options.Add(new KeyValuePair<int, Grid.RelativeCell>(q, n));
            ...
            public static Grid.RelativeCell GetBeggingForEnergyNeighborTechnite(Grid.CellID location)
            {
                return EvaluateBeggingForEnergyNeighborTechnites(location, (relative, technite) =>
                {
                    ...
                    if (needMoreEnergy)
                        return 1;
                    else return NotAChoice;
                });
            }

            public static bool isThereNothingAboveMe(Grid.CellID target)
            {
                Grid.Content content = Grid.World.GetCell(target).content;
                if (content != Grid.Content.Clear && content != Grid.Content.Water)
                    return false;
                ...
                else return isThereNothingAboveMe(target.TopNeighbor);
            }
        ...
        public static void ProcessTechnites()
        {
            ...
            foreach (Technite t in Technite.All)
            {
                ...
                if (t.selfTransform && !Technite.someoneOutThereBeggingForEnergy)
                {
                    ...
                    case MyState.growHorizontally: // grow horizontal
                        {
                                ...
                                if (target != Grid.RelativeCell.Invalid)
                                {
                                    ...
                                    if (absoluteTarget.IsValid)
                                    {
                                        if (!Helper.isThereNothingAboveMe(absoluteTarget.TopNeighbor))
                                            Technite.someoneOutThereBeggingForEnergy = true;
                    ...
                    case MyState.consumeAround: // If next to a mountain, consume it
                        if (t.CanConsume)
                        {
                            if (t.consumeAround)
                            {
                                ...
                                {
                                    if (Technite.someoneOutThereBeggingForEnergy) t.tryTransferEnergy = true;
                                    else t.selfTransform = true;
                                    t.consumeAround = false;
                                    t.grow_horizontally_done = false;
                                }
                            }
                        }
